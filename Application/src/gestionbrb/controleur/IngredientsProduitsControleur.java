...
private void refresh() throws ClassNotFoundException, SQLException {
	...
	while(rs.next()) {
		IngredientsProduits.getIngredientsData().add(new Ingredients(rs.getInt("idIngredient"), 
																	 rs.getString("nomIngredient"), 
																	 rs.getInt("prixIngredient"), 
																	 rs.getInt("qteRestante"), 
																	 rs.getString("nom")));
...
private void supprimerIngredient() throws ClassNotFoundException {
	...
	if (selectedIndex >= 0) {
		try {
			...
			requete.execute();
			refresh();
			FonctionsControleurs.alerteInfo("Suppression réussie", null, "La table "+selectedIngredient.getNomIngredient()+" vient d'être supprimée!");
			conn.close();
			requete.close();
		}
		catch(Exception e) {
...
private void supprimerProduit() throws ClassNotFoundException {
	...
	if (selectedIndex >= 0) {
		try {
			...
			requete.execute();
			refreshProduit();
			FonctionsControleurs.alerteInfo("Suppression réussie", null, "La table "+selectedProduit.getNomProduit()+" vient d'être supprimée!");
			conn.close();
			requete.close();
		}
		catch(Exception e) {
...
private void supprimerType() throws ClassNotFoundException {
	...
	if (selectedIndex >= 0) {
		try {
			...
			requete.execute();
			refreshType();
			FonctionsControleurs.alerteInfo("Suppression réussie", null, "La table "+selectedType.getNomType()+" vient d'être supprimée!");
			conn.close();
			requete.close();
		}
		catch(Exception e) {
	...
	if (selectedIngredient != null) {
		...
		if (okClicked) {
			bddUtil.dbQueryExecute("UPDATE `ingredients` SET `nomIngredient` = '" + selectedIngredient.getNomIngredient() + "', "
					...
					+ "`idfournisseur` = "+FonctionsControleurs.retrouveID(selectedIngredient.getFournisseur())
		...
		if (selectedProduit != null) {
			...
			if (okClicked) {
				bddUtil.dbQueryExecute("UPDATE `Produit` SET `nom` = '" + selectedProduit.getNomProduit() + "', "
						...
						+ "`idType` = " +FonctionsControleurs.retrouveID(selectedProduit.getType())
