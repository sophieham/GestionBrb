...
public class IngredientsProduitsControleur {
	...
	private void refreshProduit() throws ClassNotFoundException, SQLException {
		...
		Connection conn = bddUtil.dbConnect();
		ResultSet requete = conn.createStatement().executeQuery(
				"SELECT `idProduit`, produit.`nom`, `qte`, `description`, `prix`, produit.idType, type_produit.nom FROM `produit` INNER JOIN type_produit on produit.idType = type_produit.idType ");
		while (requete.next()) {
			IngredientsProduits.getProduitData()
					.add(new Produit(requete.getInt("idProduit"), requete.getString("nom"), requete.getInt("qte"),
							requete.getString("description"), requete.getInt("prix"),
